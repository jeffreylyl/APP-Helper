...
import android.app.helper.mvc.ProgressSender;
import android.app.helper.mvc.RequestHandle;
import android.app.helper.mvc.ResponseSender;
import android.app.helper.mvc.data.Data2;
import android.app.helper.task.imp.MemoryCacheStore;
...
public class TaskHelper<BASE_DATA> {
    ...
    private abstract class AbsAsyncTaskImp<DATA extends BASE_DATA> implements ResponseSender<DATA>, TaskImp<DATA> {
        ...
        public void sendError(final Throwable e) {
            onPostExecute(Code.EXCEPTION, e, null);
        }
        ...
        private void onPostExecute(final Code code, final Throwable e, final DATA data) {
            if (Looper.myLooper() != Looper.getMainLooper()) {
                handler.post(new Runnable() {
                    @Override
                    public void run() {
                        onPostExecuteMainThread(code, e, data);
                    }
                });
            } else {
                onPostExecuteMainThread(code, e, data);
        ...
        private void onPostExecuteMainThread(final Code code, final Throwable e, final DATA data) {
            if (!isRunning) {
                return;
            }
            isRunning = false;
            ...
            for (Data2<Object, ICallback<DATA>> data2 : selfCallBacks) {
                data2.getValue2().onPostExecute(data2.getValue1(), code, e, data);
            }
            for (ICallback<BASE_DATA> callback : callBacks) {
                callback.onPostExecute(realTask, code, e, data);
            }
        }
